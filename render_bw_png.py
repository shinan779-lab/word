...
W,H=1200,1570; im=Image.new('RGB',(W,H),'black'); d=ImageDraw.Draw(im)
...
def ball(cx,cy,k,r=27):
 ...
 if k=='O': d.ellipse(box,fill='#fafafa',outline='#222',width=2)
 elif k=='G': d.ellipse(box,fill='#777',outline='#eee',width=2)
 else:
...
for c,name in enumerate(cols):
 x=int(mx+c*cw); b=(x,hy,int(mx+(c+1)*cw),1256); shade='#c6c6c6' if c<6 else ('#e3e3e3' if c<9 else '#b0b0b0'); d.rectangle(b,fill=shade,outline='black',width=2); centertext(b,name,f27)
b=(1088,hy,1177,1256); d.rectangle(b,fill='#dedede',outline='black',width=2); centertext(b,'PCs',f27)
for x,k in [(37,'O'),(426,'Y'),(839,'G')]:ball(x,1351,k,34)
for x,shade in [(1,'#c6c6c6'),(394,'#e3e3e3'),(808,'#b0b0b0')]:d.rectangle((x,1491,x+63,1539),fill=shade,outline='#111',width=2)
